...
UserSchema.methods = {
  ...
  encryptPassword: function encryptPassword(password, callback) {
    ...
    if (!callback) {
      return _crypto2.default.pbkdf2Sync(password, salt, defaultIterations, defaultKeyLength, disg).toString('base64');
    }

    return _crypto2.default.pbkdf2(password, salt, defaultIterations, defaultKeyLength, disg, function (err, key) {
      if (err) {
        return callback(err);
      } else {
