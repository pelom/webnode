...
exports.me = me;
exports.signupvalid = signupvalid;
...
var _userNew = require('../../mailer/userNew.service');

var userNew = _interopRequireWildcard(_userNew);
...
function me(req, res, next) {
  ...
  return _user2.default.findOne({ _id: userId }, '-salt -password -activeToken').exec().then(function (user) {
...
function signupvalid(req, res) {
  var token = String(req.body.token);
  return _user2.default.findOne({ activeToken: token }, '-salt -password').exec().then(function (user) {
    // don't ever give out the password or salt
    if (!user) {
      return res.status(401).end();
    }
